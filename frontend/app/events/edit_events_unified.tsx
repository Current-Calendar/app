...
export default function EditEventsUnifiedScreen() {
  ...
  // Update webHour and webMinute when time changes
  useEffect(() => {
  ...
  const loadEventData = async () => {
    ...
    try {
      setTimeout(() => {
        // For demo purposes, auto-detect based on whether it has recurrence data
        const isRecurring = false; // Change to true to load recurring event
        
        if (isRecurring) {
          setEventType("recurring");
          setTitle(mockRecurringEventData.title);
          setDescription(mockRecurringEventData.description);
          setPlace(mockRecurringEventData.place);
          setTime(mockRecurringEventData.time);
          setSelectedCalendar(mockRecurringEventData.calendar);
          setRecurrence(mockRecurringEventData.recurrence);
          setInterval(String(mockRecurringEventData.interval));
          setDaysOfWeek(mockRecurringEventData.daysOfWeek);
          setEndType(mockRecurringEventData.endType);
          if (mockRecurringEventData.endDate) setEndDate(mockRecurringEventData.endDate);
          if (mockRecurringEventData.endCount) setEndCount(String(mockRecurringEventData.endCount));
        } else {
          setEventType("normal");
        ...
        if (mockEventData.coverUri || mockRecurringEventData.coverUri) {
          setCoverUri(mockEventData.coverUri || mockRecurringEventData.coverUri || null);
        }
        setLoading(false);
      }, 500);
    } catch (error) {
  ...
  const handleUpdate = async () => {
    ...
    try {
      setTimeout(() => {
        Alert.alert("Success", `${eventType === "recurring" ? "Recurring event" : "Event"} updated successfully`);
        setSaving(false);
      }, 800);
    } catch (error) {
